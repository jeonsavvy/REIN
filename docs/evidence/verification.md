...
- 최신 준비 리비전: `rein-00015-tfj`, 트래픽 100%
...
문제가 생기면 직전 검증 리비전 `rein-00013-dlg`으로 트래픽을 되돌릴 수 있다.
...
- PPTX: `output/presentation/REIN-Hackathon-Deck.pptx`, 390,244 bytes
- PDF: `output/pdf/REIN-Hackathon-Deck.pdf`, 8 pages, 960×540 pt, 969,777 bytes
- PDF SHA-256:
  `3D296EF1582B260EC48357AB413EA27FCF5147A79D67C67838D84F299C082EAA`

새 live 화면, 최신 거래 2건, 55개 unit test 수로 자료를 다시 만들었다. PDF 8장을
각각 PNG로 렌더링해 겹침, 잘림, wrapping, 폰트 대체, 대비, 여백, 의도하지 않은
crop을 확인했다. PPTX의 8장·234개 shape도 수치상 슬라이드 경계를 벗어나지 않았다.
...
- 크기: 8,396,315 bytes
- SHA-256:
  `96ADFE4BFE0D84DB8177BC8C57014DE26F3B65CD8229E4D2D677CF065A0C1753`
- 음량: `mean_volume=-18.8 dB`, `max_volume=-1.5 dB`
...
`faster-whisper-small` 역전사는 문장별 평균 0.972, 최종 MP4 0.961의 정규화
...
두 구간에만 현재 문구로 교체했고, 영수증 스크롤 구간에는 오버레이가 없다. 영상
파일 metadata도 Qwen3-TTS와 no voice cloning을 명시한다.
...
- 검증한 공개 commit: `1819f8e1f4cc33d5975d480ef4175f32a1e30826`
