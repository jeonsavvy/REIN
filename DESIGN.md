...
상태: 2026-07-22 포지셔닝 정리 및 데스크톱·모바일 재검토 완료
...
제품은 소비자 지갑 연결형 dApp처럼 보이지 않아야 한다. 사용자는 실행 시 목표와
상한을 승인하고, 이후 결제는 REIN 전용 Devnet 서버 지갑이 정책 안에서 자동
서명한다. 첫 화면에서 이 역할 구분이 기술 배경 없이도 이해되어야 한다.
...
첫 화면에서 다음 여섯 가지가 순서대로 이해되어야 한다.

1. 이 서비스가 하는 일
2. 사용자가 정하는 것과 에이전트가 맡는 것
3. 조사 목표와 최대 예산
4. 구매 가능한 데이터와 총가격
5. 현재 네 단계 진행 상태
6. 결론, 영수증, 근거와 한계
...
- demo 영수증과 Devnet 영수증을 같은 말로 부르지 않는다.
- 서버 지갑과 브라우저 지갑의 역할을 혼동시키지 않는다.
- A~D를 별도 경쟁 트랙처럼 말하지 않는다. 공식 주제는 단일 트랙이며 REIN은
  굳이 분류하면 Autonomous On-chain Settlement 성격이 가장 강하다.
- 오류에는 원인과 다음 복구 행동을 함께 적는다.
...
- 구현되지 않은 제어 기능이나 실시간 인프라를 암시하는 문구
- 영수증·예산·복구 방법보다 앞서는 토큰 또는 3D 장식

## 검토 기준

완료를 주장하기 전에 데스크톱과 390px 모바일을 픽셀로 렌더한다. 겹침, 잘림,
